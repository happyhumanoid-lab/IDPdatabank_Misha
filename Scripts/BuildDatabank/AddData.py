...
if __name__ == "__main__":
    ...
    if not fail_from_top and struc is None:
        gro_from_tpr = dir_tmp + '/gro_from_tpr.gro'
        try:
            exec_str = (
                f"gmx editconf -f {top} -o {gro_from_tpr}"
            )
            logger.debug(exec_str)
            os.system(exec_str)
            struc = gro_from_tpr
    ...
    R_membrane_z = 0
    if not lipids and len(lipids) > 0:
    ...
    for key_mol in sim["COMPOSITION"]:
        if key_mol == 'PROTEIN':
            print(extract_sequence_from_gro(protein))
            sim["COMPOSITION"][key_mol] = {'SEQUENCE': extract_sequence_from_gro(protein)}
        else:
